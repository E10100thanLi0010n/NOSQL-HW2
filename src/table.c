...
void resize(HashTable* ht) {
    ...
    KeyValue* newTable = (KeyValue*)malloc(sizeof(KeyValue) * newSize);
    ...
    for (int i = 0; i < ht->tablesize; i++)
    {
        ...
        {
            int index=hash(ht->table[i]->key,newSize);

            while(newTable[index].key!=NULL)
                index=(index+1)%newSize;

            newTable[index].key=ht->table[i]->key;
            newTable[index].value= ht->table[i]->value;
        }
